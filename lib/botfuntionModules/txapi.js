...
async function txapi(msg) {
  ...
  switch (msg) {
    ...
    case '经典台词':
      let obj = await txapimore('dialogue/index')
      return obj.dialogue + `--${obj.source}`
    case '每日一句':
      let obj = await txapimore('one/index')
      return obj.word
    case '文化谚语':
      let obj = await txapimore('proverb/index')
      return obj.front + '-->' + obj.behind
    case '土味情话':
